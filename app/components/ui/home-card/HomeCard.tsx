...
const HomeCard: React.FC<HomeCardProps> = ({ icon, iconColor, title, description, bgColor }) => {
  return (
    <View className='rounded-lg shadow-md p-4 mb-4 w-full flex-row items-center' style={{ backgroundColor: bgColor }}>
      <View className='flex-1 mr-4'>
        <Text className='text-xl font-semibold text-black'>{title}</Text>
        <Text className='mt-2 text-md text-gray-700'>{description}</Text>
      </View>
